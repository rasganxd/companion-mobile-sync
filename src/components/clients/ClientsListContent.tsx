...
const ClientsListContent: React.FC<ClientsListContentProps> = ({
  ...
  const filteredClients = clients.filter(client => {
    ...
    return client.name.toLowerCase().includes(searchLower) || client.company_name && client.company_name.toLowerCase().includes(searchLower) || client.phone && client.phone.includes(searchTerm) || client.address && client.address.toLowerCase().includes(searchLower) || client.city && client.city.toLowerCase().includes(searchLower) || client.code && client.code.toString().includes(searchTerm);
  });
  ...
  if (clients.length > 0 && salesRep) {
    return <div className="space-y-4">
        {/* Search bar */}
        <div className="bg-white rounded-lg shadow p-4 px-[10px] py-[10px]">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400" />
            <input type="text" placeholder="Buscar por nome, empresa, telefone, endereço..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-app-blue focus:border-transparent" />
          </div>
        </div>
        
        {filteredClients.length > 0 ? <>
            {viewMode === 'paginated' ? <ClientPaginatedView clients={filteredClients} currentPage={currentPage} onPageChange={setCurrentPage} onClientSelect={onClientSelect} onToggleView={toggleViewMode} onViewOrder={onViewOrder} /> : <div className="space-y-4">
                {/* Botão para alternar para modo paginado */}
                <div className="flex justify-end">
                  <AppButton variant="gray" onClick={toggleViewMode} className="flex items-center gap-2 px-[5px] py-[5px]">
                    <User size={16} />
                    <span className="text-xs">Ver Páginas</span>
                  </AppButton>
                </div>
                
                <div className="space-y-3">
                  {filteredClients.map(client => <ClientCard key={client.id} client={client} onSelect={onClientSelect} onViewDetails={handleViewDetails} onViewOrder={onViewOrder} />)}
                </div>
              </div>}
