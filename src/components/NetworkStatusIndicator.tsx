...
const NetworkStatusIndicator: React.FC = () => {
  ...
  return <div className="flex items-center gap-2 text-xs">
      <Badge variant="secondary" className={`flex items-center gap-1 px-2 py-1 ${getStatusColor()}`}>
        {getStatusIcon()}
        <span>{getStatusText()}</span>
      </Badge>
      
      {lastSyncDate && <Badge variant="outline" className="px-2 py-1">
          <span className="text-slate-50">Sync: {formatLastSync()}</span>
        </Badge>}
      
      {!connected && <Badge variant="outline" className="px-2 py-1 bg-blue-50 text-blue-700">
